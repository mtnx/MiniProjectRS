...
func main() {
	...
	var server pb.ObatServiceServer
	{
		...
		service := svc.NewObat(dbReadWriter)
		endpoint := ep.NewObatEndpoint(service)
		fmt.Println(endpoint)
		server = ep.NewGRPCObatServer(endpoint, tracer, logger)
	}
	...
	pb.RegisterObatServiceServer(grpcServer, server)
